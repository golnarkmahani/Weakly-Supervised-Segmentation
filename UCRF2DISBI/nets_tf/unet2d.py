...
class UNet2D(Model):
    ...
    def __init__(self, n_class, n_layer, root_filters, kernal_size=3, pool_size=2, use_bn=True, use_res=True, padding='SAME', concat_or_add='concat'):
        ...
        for layer in range(n_layer-2, -1, -1):
            ...
            dict_key = str(n_layer-layer-1)
            up = _UpSampling(filters, kernal_size, pool_size, 'up_%d'%layer, concat_or_add, use_bn, use_res)
    ...
    @tf.function
    def __call__(self, x_in, drop_rate=0, training=False, feature_last_conv= False):
        dw_tensors = dict()
        x = x_in
        ...
        features_before_last_cov =x
        x = self.conv_out(x)
        x = tf.nn.relu(x)
        if feature_last_conv:
            return x, features_before_last_cov
        else:
            return x
...
class _DownSampling(layers.Layer):
    ...
    def __call__(self, x_in, drop_rate, training=True):
        # conv1
        x = self.conv1(x_in)
        # x = tf.nn.dropout(x, drop_rate, noise_shape=)
        x = layers.SpatialDropout2D(drop_rate)(x)
        # another version for spatial dropout
        # tf.nn.dropout(x, drop_rate, x.shape[0], 1, 1, x.shape[-1])
        if self.use_bn:
