...
class Trainer:
    ...
    def train(self, 
        ...
        print('Start training: epochs {}, learning rate {}, batch size {}, mini-batch size {}, training data {}, validation data {}.'
        ...
        best_epochs = {'bloss':0,'bdice':0, 'bhaus':0}
        for ep in range(epochs):
            ...
            # TODO: define a new function for saving necessary measures for learning curve
            tr_temp = ep_train_eval
            val_temp = ep_valid_eval
            if 'logits' in tr_temp:
                tr_temp.pop('logits')
            if 'imgs' in tr_temp:
                tr_temp.pop('imgs')
            if 'pred' in tr_temp:
                tr_temp.pop('pred')
            if 'logits' in val_temp:
                val_temp.pop('logits')
            if 'imgs' in val_temp:
                val_temp.pop('imgs')
            if 'pred' in val_temp:
            ...
            train_eval_str = U.dict_append(train_eval_str, tr_temp)
            valid_eval_str = U.dict_append(valid_eval_str, val_temp)
        
            # TODO add early stopping and best ckpt save
            # TODO add tensorboard summary
            ckpt.write(output_path + '/ckpt/final')

            #To save the best epochs
            with open(output_path + '/train_log.txt', 'a+') as f:
                f.write(str(best_epochs) + '\n')
