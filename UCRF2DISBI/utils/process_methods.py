...
def channel_lab_check(x, n_lab_channel):
    _x = np.zeros(list([x.shape[0],x.shape[1],x.shape[-1]]))
    if x.shape[-2] ==3:
        _x[..., 0] = x[..., 0, 0]
        _x[..., 1] = x[..., 0, 1]
    else:
    ...
    if not x.shape:
        _x = np.expand_dims(_x, -1)
    elif x.shape[-1] != n_lab_channel:
        assert n_lab_channel == 1 and x.shape[-1] != 3, 'Expect channel {}, actual shape {}.'.format(n_lab_channel, x.shape)
        _x = np.reshape(x, list(x.shape) + [1])
    return _x
